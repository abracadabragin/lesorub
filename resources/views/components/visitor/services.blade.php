<section class='jumbotron mb-0' id='services'>

    <div class='container'>

        <div class='row justify-content-center mb-4'>

            <h1 class='section-header'>Услуги</h1>

        </div>

        <div class='row'>

            <div class='col-md-3 d-flex flex-column align-items-center'>
                <div class='service-icon'>
                    <img src='{{ URL::asset('img/icons/home.png') }}'
                         alt='Systems'>
                </div>
                <div class='service-title mt-3 mb-2'>
                    <h5>Фундамент</h5>
                </div>
                <div class='service-description'>
                    <ul>
                        <li>Блочный фундамент</li>
                        <li>Винтовые сваи</li>
                        <li>Ленточный фундамент</li>
                        <li>Сваи ТИСЭ</li>
                        <li>Бураналивные сваи</li>
                    </ul>
                </div>
            </div>

            <div class='col-md-3 d-flex flex-column align-items-center'>
                <div class='service-icon'>
                    <img src='{{ URL::asset('img/icons/paint-brush.png') }}' alt='Foundation'>
                </div>
                <div class='service-title mt-3 mb-2'>
                    <h5>Отделочные работы</h5>
                </div>
                <div class='service-description'>
                    <ul>
                        <li>Шлифовка</li>
                        <li>Покраска</li>
                        <li>Теплый шов</li>
                        <li>Окосячка (обсадка проемов)</li>
                        <li>Установка пластиковых окон</li>
                    </ul>
                </div>
            </div>

            <div class='col-md-3 d-flex flex-column align-items-center'>
                <div class='service-icon'>
                    <img src='{{ URL::asset('img/icons/portfolio-black-tool.png') }}' alt='Portfolio'>
                </div>
                <div class='service-title mt-3 mb-2'>
                    <h5>Дополнительные услуги</h5>
                </div>
                <div class='service-description'>
                    <ul>
                        <li>Беседки</li>
                        <li>Ограждения</li>
                        <li>Сточная система</li>
                        <li>Отмостка</li>
                        <li>Сантех услуги</li>
                        <li>Электромонтажные работы</li>
                    </ul>
                </div>
            </div>

            <div class='col-md-3 d-flex flex-column align-items-center'>
                <div class='service-icon'>
                    <img src='{{ URL::asset('img/icons/courthouse.png') }}' alt='Law'>
                </div>
                <div class='service-title mt-3 mb-2'>
                    <h5>Юридическая помощь</h5>
                </div>
                <div class='service-description'>
                    <ul class='ml-3'>
                        <li>Разрешение на строительство</li>
                        <li>Оформление жилого дома</li>
                        <li>Дачная амнистия</li>
                        <li>Кадастровые работы</li>
                        <li><a href='http://citygroup54.ru/' target='_blank'>Переход на сайт</a></li>
                    </ul>
                </div>
            </div>

        </div>

    </div>

</section>